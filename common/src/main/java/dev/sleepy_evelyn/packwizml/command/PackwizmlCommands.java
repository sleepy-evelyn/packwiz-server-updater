...
public final class PackwizmlCommands {
    ...
    private static final MutableText AUTO_UPDATE_ON = Text.literal("Enabled automatic modpack updates").formatted(Formatting.GREEN);
    private static final MutableText AUTO_UPDATE_OFF = Text.literal("Disabled automatic modpack updates").formatted(Formatting.RED);
    ...
    private static int toggleAutoUpdate(CommandContext<ServerCommandSource> ctx) throws CommandSyntaxException {
        boolean autoUpdate = BoolArgumentType.getBool(ctx, "autoUpdate");
        setAndUpdateConfigValue("auto_update", String.valueOf(autoUpdate));
        if (autoUpdate)
            getCommandOutput(ctx).sendMessage(AUTO_UPDATE_ON);
        else
            getCommandOutput(ctx).sendMessage(AUTO_UPDATE_OFF);
        return Command.SINGLE_SUCCESS;
    }
