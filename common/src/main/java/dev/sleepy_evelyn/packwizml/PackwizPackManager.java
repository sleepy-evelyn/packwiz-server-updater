...
public final class PackwizPackManager {
    ...
    public void update() {
        update(CONFIG_HANDLER.getValue("pack_toml"), hasBootstrap(), null);
    }

    public void update(String packTomlLink, boolean hasBootstrap, @Nullable CommandContext<ServerCommandSource> ctx) {
        List<String> command = new ArrayList<>(PACKWIZ_COMMAND_PREFIX);
        boolean isDedicatedServer = PackwizmlPlatform.isDedicatedServer();
        ...
        if (!HAS_TASK.test(UPDATE_PACKWIZ_TASK_NAME)) {
            ...
            // TODO: Add a check for first launch here and add a confirmation when it finishes
            if (ctx != null)
                TASKS.add(new PackwizmlCommands.AsyncCommandTask(CompletableFuture.runAsync(commandTask),
                        UPDATE_PACKWIZ_TASK_NAME, 20, ctx));
            else commandTask.run();
        }
    }
